...
if [ "$RUNNER_OS" = "macOS" ]; then
  ...
  echo "HEX_CACERTS_PATH=/Users/mitmproxyuser/.mitmproxy/mitmproxy-ca-cert.p12" >> $GITHUB_ENV
  ...
  gh_api_ip=`getent hosts api.github.com | awk '{ print $1 }'`
  sudo -u mitmproxyuser -H bash -e -c "cd /home/mitmproxyuser && /home/mitmproxyuser/.local/bin/mitmdump --mode transparent --allow-hosts '$gh_api_ip' --showhost --set block_global=false -q -s /home/mitmproxyuser/mitm_plugin.py --set output='/home/mitmproxyuser/out.txt' --set token='$INPUT_TOKEN' --set hosts='api.github.com,github.com' --set debug='$RUNNER_DEBUG' --set ACTIONS_ID_TOKEN_REQUEST_URL='$ACTIONS_ID_TOKEN_REQUEST_URL' --set ACTIONS_ID_TOKEN_REQUEST_TOKEN='$ACTIONS_ID_TOKEN_REQUEST_TOKEN' --set GITHUB_REPOSITORY_ID='$GITHUB_REPOSITORY_ID' --set GITHUB_REPOSITORY='$GITHUB_REPOSITORY' &"
  ...
  # echo "MAVEN_OPTS=-Djavax.net.ssl.trustStore=/home/mitmproxyuser/.mitmproxy/mitmproxy-ca-cert.pem -Djavax.net.ssl.trustStoreType=pkcs12 -Djavax.net.debug=ssl,keymanager" >> $GITHUB_ENV
